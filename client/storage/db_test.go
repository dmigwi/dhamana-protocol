...
func TestQueryLocalData(t *testing.T) {
	...
	// Postgres timestamp is formatted by default to timezone Etc/UTC
	loc, _ := time.LoadLocation("Etc/UTC")
	maturityDate, _ := pq.ParseTimestamp(nil, "2024-08-02 00:00:00.501361+03")
	...
	dataExp := servertypes.BondByAddressResp{
		...
		Principal:       14000,
		CouponDate:      2,
		MaturityDate:    maturityDate.In(loc),
	...
	t.Run("Test GetBondByAddress result", func(t *testing.T) {
		// Addresses in the variadic parameter must be provided to this methods as raw strings
		// instead of type common.Address{}. This prevents the following error from happening:
		// pq: invalid byte sequence for encoding "UTF8": 0xc0 0x2a

		data, err := db.QueryLocalData(utils.GetBondByAddress, new(servertypes.BondByAddressResp),
			sender, "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756dba")
		if err != nil {
			t.Fatalf("expected no error but found: %v", err)
		}
		...
		compare((*res).MaturityDate, dataExp.MaturityDate)
	})
	...
	t.Run("Test GetChats results", func(t *testing.T) {
		data, err := db.QueryLocalData(utils.GetChats, new(servertypes.ChatMsgsResp), sender,
			"0xc02aaa39b223fe8d0a0e5c4f27ead9083c756dba", limit, offset)
		if err != nil {
			t.Fatalf("expected no error but found: %v", err)
		}
	...
	blockExp := servertypes.LastSyncedBlockResp(89)
	...
	t.Run("Test GetLastSyncedBlock result", func(t *testing.T) {
		...
		res := data[0].(*servertypes.LastSyncedBlockResp)

		compare(*res, blockExp)
	})
}
